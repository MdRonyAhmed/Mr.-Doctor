...
class make_appoint:
    ...
    def appointment_form(self,list):
        doct_id = []
        for i in list.curselection():
            id = list.get(i)
            doct_id.append(id)
        appoint = appointment.appoint_form()
        appoint.swap_page(doct_id)

    def Available_Day(self,idList):
    ...
    def Elements(self):
        ...
        lsit = self.Available_Day(doctor_id_list)
        tk.Button(Frame, text="Appointment",command=lambda:self.appointment_form(lsit),fg="White",bg=ButtonBackground_color,font=("Open sans",15)).place(x=180,y=550)
